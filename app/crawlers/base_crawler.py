...
from app.crawlers.api_exceptions import (
    APIError,
    APIConnectionError,
    APIResponseError,
    APITimeoutError,
...
class BaseCrawler:
        ...
        if (
            ...
            try:
                ...
                try:
                    ...
                    self.logger.error("解析 {0} 接口 JSON 失败： {1}".format(response.url, e))
            ...
            if isinstance(response, Response):
                ...
                self.logger.error("无效响应类型。响应类型: {0}".format(type(response)))
    ...
    async def get_fetch_data(self, url: str):
        ...
        for attempt in range(self._max_retries):
            try:
                ...
                # self.logger.info("响应状态码: {0}".format(response.status_code))
    ...
    def handle_http_status_error(self, http_error, url: str, attempt):
        ...
        if response is None or status_code is None:
            self.logger.error("HTTP状态错误: {0}, URL: {1}, 尝试次数: {2}".format(
